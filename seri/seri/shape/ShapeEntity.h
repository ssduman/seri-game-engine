...
namespace seri
{
	class ShapeEntity : public Object
	{
	public:
		ShapeEntity() = delete;

		ShapeEntity(std::shared_ptr<CameraBase>& camera) : _camera(camera)
		{
			_shader = ShaderBase::Create();
			_texture = TextureBase::Create();
		}

		ShapeEntity(ShapeEntity&& other) = default;

		ShapeEntity(const ShapeEntity& other) = delete;

		ShapeEntity& operator=(ShapeEntity&& other) = default;

		ShapeEntity& operator=(const ShapeEntity& other) = delete;

		~ShapeEntity() override
		{
		}
