	...
	void ShaderLibrary::Init(const char* shaderFolderPath)
	{
		...
		for (const auto& entry : std::filesystem::directory_iterator(shaderFolderPath))
		{
			...
			else if (Util::Contains(path, ".glsl"))
			{
				...
				{
					LIB_LOGGER(error, shader_library) << fmt::format("glsl include not supported in glsl: {}", name);
					continue;
				}
		...
		LIB_LOGGER(info, shader_library) << fmt::format("init done, glsl count: {}, shader count: {}", GetInstance()._predefinedGLSLs.size(), GetInstance()._predefinedShaders.size());
	}
