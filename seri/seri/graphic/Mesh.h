...
namespace seri
{
	...
	public:
		...
		void UpdateAnimation(const NodeData& node, const glm::mat4& parentTransform)
		{
			...
			if (bones.size() > 0)
			{
				auto boneIt = boneNameToIndexMap.find(nodeName);
				int boneIndex = boneIt != boneNameToIndexMap.end() ? boneIt->second : -1;

				if (boneIndex != -1 && bones.find(boneIndex) != bones.end())
				{
					if (bones.at(boneIndex).name != nodeName)
					{
						LIB_LOGGER(error, mesh) << "bone name mismatch: " << nodeName << ", " << bones.at(boneIndex).name;
					}
					else
					{
		...
		void Build()
		{
			if (_vao)
			{
				//LIB_LOGGER(warning, mesh) << "vao already generated for build";
				return;
			}
		...
		void Update()
		{
			...
			{
				Build();
				//LIB_LOGGER(error, mesh) << "vao for update not generated";
				return;
			}
		...
		void MakeInstanced(const std::vector<glm::mat4>& modelMatrices)
		{
			...
			{
				LIB_LOGGER(error, mesh) << "model matrices size exceeds max instanced count";
				return;
			}

			if (!_vao)
			{
				LIB_LOGGER(error, mesh) << "vao for instanced not generated";
				return;
			}

			if (_vbo_instanced)
			{
				LIB_LOGGER(error, mesh) << "vbo for instanced already generated";
				return;
			}
		...
		void UpdateInstanced(const std::vector<glm::mat4>& modelMatrices)
		{
			...
			if (!_vbo_instanced)
			{
				LIB_LOGGER(error, mesh) << "vbo for instanced not generated";
				return;
			}
