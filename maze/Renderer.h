...
public:
    Renderer(float width, float height, float depth) : Entity(nullptr), _width(width), _height(height), _depth(depth) {
        LOGGER(info, "renderer init succeeded");
    }
    ...
    ~Renderer() override {
        ...
        LOGGER(info, "renderer delete succeeded");
    }

    void setCoordinates(unsigned int count, const std::vector<glm::vec3>& coordinates) {
        _count = count;
    ...
    void setProperties(const std::vector<int>& indices, const std::vector<float>& vertices) {
        _indices = indices;
        _vertices = vertices;
    }
...
private:
    void setProperties() {
        const float w = _width, h = _height, z = _depth;
